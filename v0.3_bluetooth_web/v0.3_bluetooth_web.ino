...
//#define Inclinaison_offset 1//sensibiliter du gauge inclinaison
...
int directions, angles, temps, acceleration, acc, angles_max(0);
...
void setup() {
  //Demarrage ecran oled
  u8g2.begin();
  //Boot Screen avec logo ninja
  u8g2.firstPage();
   do {
...
//----------------------------------------------MAIN
void loop() {
  //Attend d'une connection
  BLEDevice central = BLE.central();
  
  angles = complementaryPitch; 
  acceleration = accelY*Acceleration_offset;
  u8g2.firstPage();
  do {
...
void doCalculations() {
  ...
  if(millis()<acc_angle + 10000){
    complementaryRoll = 0.95 * complementaryRoll + 0.05 * accRoll;
    complementaryPitch = 0.95 * complementaryPitch + 0.05 * accPitch;
  }
  else{
    complementaryRoll = 0.995 * complementaryRoll + 0.005 * accRoll;
    complementaryPitch = 0.995 * complementaryPitch + 0.005 * accPitch;
  }
}
//----------------------------------------------UPDATE INFO BLUETOOTH 
//Temporaire
